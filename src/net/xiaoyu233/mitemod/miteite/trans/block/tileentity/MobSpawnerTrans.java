...
public abstract class MobSpawnerTrans {
   ...
   public void updateSpawner() {
      if (this.canRun()) {
         boolean playerBoost = Configs.wenscConfig.mobSpawnerSpeedUpWithPlayers.ConfigValue && !this.getSpawnerWorld().getEntitiesWithinAABB(EntityPlayer.class, AxisAlignedBB.getAABBPool().getAABB(this.getSpawnerX(), this.getSpawnerY(), this.getSpawnerZ(), this.getSpawnerX() + 1, this.getSpawnerY() + 1, this.getSpawnerZ() + 1).expand(this.spawnRange * 2, this.spawnRange, this.spawnRange * 2)).isEmpty();
         double var5;
